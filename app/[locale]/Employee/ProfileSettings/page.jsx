...
const ProfileSettings = () => {
  ...
  const validationSchema = yup.object().shape({
    ...
    Password: yup.string().test('password-match', 'Les mots de passe doivent correspondre', function(value) {
      return !value || value === this.parent.confirmPassword;
    }),
    confirmPassword: yup.string().test('password-match', 'Les mots de passe doivent correspondre', function(value) {
  ...
  const sendUserData = async (values, setSubmitting) => {
    try {
      ...
      const response = await axiosInstance.patch(`/user/updateUser?id=${userInfo?._id}`, updateData);
      ...
      showPopupMessage("An error occurred while updating the user: " + (error.response?.data?.message || error.message), false);
        ...
        <div className="flex flex-col m-auto md:w-2/3 h-fit bg-white p-2 sm:p-10 rounded-b-lg rounded-bl-lg shadow-md">
              ...
              <form
                ...
                <div className="w-full mt-4">
                  ...
                  {formik.touched.phoneNumber && formik.errors.phoneNumber && (
                    <p className="text-red-500 ml-6">{formik.errors.phoneNumber}</p>
                  )}
                </div>

                <p className="w-full text-left ml-6 mt-4 text-gray-500">(Optionnel)</p>
                <div className="w-full mt-4">
                  <div className="flex ml-6 mr-6 justify-start items-center px-2 input border-b border-[#314155] h-10">
                    <FaLock className="text-gray-500 mr-2" />
                    <p className="text-l mr-2 whitespace-nowrap">Mot de passe actuel :</p>
                  ...
                  <p className="mb-4 text-red-500">
                    {formik.errors.Passwordactuel && formik.touched.Passwordactuel && formik.errors.Passwordactuel}
                  </p>
                </div>
                <div className="w-full mt-4">
                  <div className="flex ml-6 mr-6 justify-start items-center px-2 input border-b border-[#314155] h-10">
                    <FaLock className="text-gray-500 mr-2" />
                    <p className="text-l mr-2 whitespace-nowrap">Nouveau mot de passe :</p>
                    <div className="flex-grow flex items-center">
                      <input
                        className="px-4 w-full focus:outline-none"
                ...
                <div className="w-full mt-4">
                  <div className="flex ml-6 mr-6 justify-start items-center px-2 input border-b border-[#314155] h-10">
                    <FaLock className="text-gray-500 mr-2" />
                    <p className="text-l mr-2 whitespace-nowrap">Confirmer mot de passe :</p>
                    <div className="flex-grow flex items-center">
                      <input
                        className="px-4 w-full focus:outline-none"
              ...
              {showPopup && (
                <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50 z-50">
                  <div className="bg-white p-6 rounded-lg">
                    <p>{popupMessage}</p>
                    <button
                      className={`mt-4 ${isSuccess ? 'bg-green-500' : 'bg-red-500'} text-white px-4 py-2 rounded`}
          ...
          <div className="space-y-4 text-black pb-10">
            ...
            <div className="flex items-center ml-6 mr-6 border-b border-[#314155] pb-4">
              ...
              <label htmlFor="phone" className="hidden sm:block font-medium text-gray-700 w-26">
                Tél :
              </label>
