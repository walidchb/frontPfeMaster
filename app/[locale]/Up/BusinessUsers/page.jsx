  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, actions) => {
      try {
        ...
        const userExistsResponse = await axiosInstance.post("/user/check-user-exists", {
          email: values.email,
          phoneNumber: values.phoneNumber,
        });
  
        if (userExistsResponse.data.exists) {
          setErrorCred({ userExist: "User with this email or phone number exists" });
          return;
        }
  
        // Créer l'utilisateur dans Firebase
        const firebaseUserCredential = await createUserWithEmailAndPassword(auth, values.email, values.password);
        ...
        const orgResponse = await axiosInstance.post("/organization/organizations", {
          email: values.email,
        ...
        const updatedUserResponse = await axiosInstance.patch(`/user/users?id=${userInfo._id}`, {
        ...
        try {
          ...
          if (organizationInfo) {
            ...
            await axiosInstance.delete(`/organization/organizations/${organizationInfo._id}`);
          }
        } catch (cleanupError) {
