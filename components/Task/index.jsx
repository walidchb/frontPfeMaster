...
import { FaChevronDown, FaChevronUp, FaCalendarAlt, FaClipboardList, FaUserFriends, FaEye } from 'react-icons/fa';
...
const Task = (task) => {
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow-md p-4 mb-4 ${isDragging ? 'dragging' : ''}`} // Appliquer la classe CSS conditionnellement
      draggable
    ...
    >
      <div className="flex justify-between items-center mb-2">
        ...
        <span className={`px-2 py-1 rounded-full ${priorityClass}`}>{task.priority}</span>
      </div>
      <div className={`${showMore ? 'block' : 'hidden'}`}>
        ...
        <div className="flex items-center mb-2">
          ...
          <span className="text-sm text-gray-600">Équipe: {task.team.name}</span>
        </div>
      </div>
