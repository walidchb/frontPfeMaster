...
const KanbanBoard = () => {
  const [tasks, setTasks] = useState([
    {
      ...
      status: 'todo',
      comments: [
          { author: 'John Doe', date: '2023-05-01 10:30', text: 'Premier commentaire' },
          { author: 'Jane Smith', date: '2023-05-02 14:45', text: 'Deuxième commentaire' }]
    },
    {
      id: 2,
      title: 'Tâche 2',
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-[url('/BG.jpeg')]">
      <div className="w-full lg:ml-50 p-4">
        <h1 className="text-3xl font-bold mb-8">Tasks</h1>
        <div className="flex flex-wrap -mx-4">
          <Column title="Todo" tasks={getTasksForStatus('todo')} onDrop={onDrop} onDragOver={onDragOver} status='todo' />
          <Column title="In progress" tasks={getTasksForStatus('doing')} onDrop={onDrop} onDragOver={onDragOver} status='doing'/>
          <Column title="Done" tasks={getTasksForStatus('done')} onDrop={onDrop} onDragOver={onDragOver} status='done'/>
        </div>
      </div>
    </div>
  );
